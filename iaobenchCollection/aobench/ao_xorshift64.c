...
static XorShift64 *randptr, randstruct;
...
static void ambient_occlusion(vec *col, const Isect *isect)
{
    ...
    for (j = 0; j < ntheta; j++) {
        for (i = 0; i < nphi; i++) {
            double theta = sqrt(xorshift64NextDouble(randptr));
            double phi   = 2.0 * M_PI * xorshift64NextDouble(randptr);
...
{
    randptr = &randstruct;
    xorshift64Init(randptr, 12345678910);
