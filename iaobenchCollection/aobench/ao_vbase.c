// make: llvm-gcc -o ao_vbase -O3 -mfpmath=sse -msse2 ao_vbase.c -lm
...
static void vmults(vec *c, vec v, double n)
{
    c->x = v.x * n;
    c->y = v.y * n;
    c->z = v.z * n;
...
static void vmultsadd(vec *c, vec v0, double n, vec v1)
{
    c->x = v0.x * n + v1.x;
    c->y = v0.y * n + v1.y;
    c->z = v0.z * n + v1.z;
...
static void vdivs(vec *c, vec v, double n)
{
    c->x = v.x / n;
    c->y = v.y / n;
    c->z = v.z / n;
...
static void ambient_occlusion(vec *col, const Isect *isect)
{
    ...
    vec p;
    vmultsadd(&p, isect->n, eps, isect->p);
/*
...
*/
            ...
            vdivs(&fimg[y * w + x], fimg[y * w + x], (double)(nsubsamples * nsubsamples));
/*
