// make: llvm-gcc -o ao_gcd -O3 ao_gcd.c -lm
// -mfpmath=sse -msse2 
// cc -Wall -o ao_gcd ao_gcd.c -lm
...
static void
render_tile(unsigned char *img, int comps, double *fimg, int w, int h, int nsubsamples, int tilex, int tiley, int tilew, int tileh)
{
...
render(unsigned char *img, int comps, int w, int h, int nsubsamples, int tilew, int tileh)
{
    ...
    // dispatch_release(dgroup); // This is not available in ARC.
}
