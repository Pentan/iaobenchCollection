// make: llvm-gcc -o ao_f_xorshift -O3 -mfpmath=sse -msse2 ao_f_xorshift.c -lm
// cc -Wall -o ao_f_xorshift ao_f_xorshift.c -lm
// clang -Wall -O3 -o ao_f_xorshift ao_f_xorshift.c -lm <- faster than llvm-gcc in this case.
...
static XorShift128 randstruct, *randptr;
//
...
static void ambient_occlusion(vec *col, const Isect *isect)
{
    ...
    for (j = 0; j < ntheta; j++) {
        for (i = 0; i < nphi; i++) {
            float theta = sqrt(xorshift128NextFloat(randptr));
            float phi   = 2.0 * M_PI * xorshift128NextFloat(randptr);
...
{
    randptr = &randstruct;
    xorshift128Init(randptr, 123456789);
...
main(int argc, char **argv)
{
    ...
    randptr = &randstruct;
    randomizerInit32(randptr, 123456789);
